...
namespace AttendanceSystem.ImportFile.API.Services.AttendanceServices
{
    public interface IAttendanceService
    {
        ...
        Task<Employee?> GetEmployeeByCodeAsync(string code, ApplicationDbContext db);//
        Task<(bool Success, string Message, string? ImageUrl)> UploadProfileImageAsync(string code, IFormFile file, ApplicationDbContext db, HttpContext httpContext);//
